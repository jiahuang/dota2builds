...
def csvToSql():
  data = csv.reader(open('dota2buildsspreadsheet2.csv'))
  # Read the column names from the first line of the file
  fields = data.next()
  buildsSql = open("dbPopulateBuilds2.sql", "a")
  for row in data:
    ...
    heroSql = '''INSERT INTO tbl_builds (descriptionOverall, descriptionEarly, descriptionMid, descriptionLate, name, whereFrom, whereUrl, hero, rating)
      VALUES ('''+row[3]+", "+row[4]+", "+row[5]+", "+row[6]+", "+row[2]+", "+row[7]+", "+row[8]+", "+row[1]+", "+row[9]+');\n'
